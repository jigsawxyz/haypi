...
module.exports = function (info) {
    var context = this;
    if (!context.name) {
        throw new Error("Please specify a name for the app ex. `haypi.name = 'My App'`")
    }
    ...
    context.events.init.call(context, app, function(){
        var schemaHelpers = require('./schemaHelpers')(context);

        var docs = require('json-schema-docs')({ title: context.name, schema: schemaHelpers.formatSchemaForDocs(_.cloneDeep(context.schemas)) });
        ...
        // catch and send 404
        const errors = require('./errors')(context);
        var notFoundError = errors.notFound;
        ...
        */
        ...
        context.events.start(function(err){
            if(err){
                throw err;
            }
