...
module.exports = (context, opts) => {
    let requestHandler = require('./requestHandler')(context, opts);
    ...
    let schemaHelpers = {
            ...
            _.forEach(_.keys(schema), function (key) {
                ...
                if (routeErrors) {
                    ...
                    _.forEach(routeErrors, (err) => {
                        var errObj = new err()
                        if (_.difference([ 'type', 'httpCode', 'internalCode' ], _.keys(errObj)).length > 0) {
                            throw new Error(`Invalid response error in schema ${schema.resource}/${val.name}, error => ${_.get(errObj, 'message', errObj)}`)
                        }
                        val.response.examples.push({
                            status: {
                                type: errObj.type,
                                code: errObj.httpCode,
                            },
                            properties: requestHandler.formatError(errObj),
                        });
                    });
                }
        ...
        buildMockRoutesFromSchema: (parentRouter, schema, parentSchema) => {
            let router = Router({ mergeParams: true });
            ...
            _.forEach(schemaKeys, (key) => {
                ...
                if (route.rootUri) {
                    schemaHelpers.buildMockRoutesFromSchema(router, schema[key], schema);
                }
                ...
                if (route.uri && route.method) {
                    ...
                    router[route.method.toLowerCase()](route.uri, function (req, res) { res.status(routeSchema.response.meta.code).json(jsf(routeSchema.response)) });
                }
            });
